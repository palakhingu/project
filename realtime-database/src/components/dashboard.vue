...
export default {
  ...
  deleteUserData(userId, index) {
    axios.delete(`https://fir-auth-15721-default-rtdb.firebaseio.com/user/${userId}.json`)
      .then(() => {
  ...
  deleteUser(post, index) {
    ...
    getdata() {
      axios.get("https://fir-auth-15721-default-rtdb.firebaseio.com/user.json")
    .then((res) => {
        ...
        <table class="table table-striped">
          ...
          <tbody>
            <tr v-for="(post, index) in postData" :key="post.id">
              ...
              <td>{{ post.dob }}</td>
              <td>
                <button class="btn btn-primary" @click="editUser(post.id)">
                  Edit
                </button>
                &nbsp;
                <button
                  class="btn btn-danger"
                  @click="deleteUser(post,index)"
                >
                  Delete
                </button>
              </td>
